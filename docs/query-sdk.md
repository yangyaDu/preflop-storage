...
本文档说明业务侧如何通过查询服务读取策略数据。当前推荐 **RangeStrataQueryService**（Rust 热路径 + .idx/.bin 文件），性能远优于旧方案。

## 推荐方案：RangeStrataQueryService（方案二 + Rust）
...
```ts
import { PreflopQueryService } from "../src/scheme1/query/preflop-query-service";
