...
| `src/range-strata-binary/compiler/pipeline.ts` | Range Strata Binary 构建、manifest、resume、stats 核心实现 |
...
| `src/range-strata-binary/query/service.ts` | 推荐查询 SDK |
| `src/range-strata-binary/benchmark/runner.ts` | Range Strata Binary benchmark 运行器 |
| `src/range-strata-binary/cli/cold-benchmark.ts` | Range Strata Binary fresh process 冷启动 benchmark |

### Scheme1 相关关键文件

| 文件 | 作用 |
| --- | --- |
| `src/scheme1/cli/compile.ts` | Scheme1 构建 CLI |
| `src/scheme1/importer/build-binary-store.ts` | Scheme1 构建核心实现 |
| `src/scheme1/query/preflop-service.ts` | Scheme1 查询服务 |
| `src/scheme1/cli/verify.ts` | 旧 SQLite 和 Scheme1 二进制一致性校验 |
