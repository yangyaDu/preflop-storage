...
| `src/scheme1/` | 已废弃旧方案（兼容和对比基线）：SQLite 索引 + `.bin` |
...
Scheme1 自 2026-06-21 起已标记为 deprecated。它只保留给旧数据兼容、SQLite 基线对比和历史校验；新构建、查询、校验和 benchmark 都应使用 `src/range-strata-binary/`。Scheme1 计划保留到 `0.2.x` 兼容窗口结束，目标在 `0.3.0` 移除。
...
`package.json` 中的构建、查询、校验和 benchmark 脚本现在统一通过 `src/cli/main.ts` 分发。常用别名仍然是 `bun run build`、`bun run query`、`bun run verify`、`bun run benchmark`；需要显式选择旧路径时使用 `bun run cli <command> --scheme scheme1`。
...
| `bun run check:release` | 发布前检查：Scheme1 deprecation notice + `check` + `check:native` + Range Strata Binary standalone CRC 自检 |
| `bun run cli <command>` | 统一 CLI 分发入口，默认选择 Range Strata Binary |
...
```text
src/cli/main.ts build
```

统一入口默认分发到 `src/range-strata-binary/cli/compile.ts`。
...
- 旧数据兼容
- SQLite / Scheme1 历史基线对比
- 迁移期间的回归校验

新构建和查询应使用 Range Strata Binary 的 `bun run build` 和 `bun run query`。Scheme1 目标在 `0.3.0` 移除。

#### 构建

```powershell
bun run cli build --scheme scheme1 --source range-db/range.db --out range-db/binary --overwrite
```

#### 查询

```powershell
bun run cli query --scheme scheme1 --dir range-db/binary --player-count 6 --depth-bb 100 --concrete-line-id 1 --hand AA
```
...
```powershell
bun run cli analyze-sqlite --source range-db/range.db --out reports/sqlite-analysis.json --md reports/sqlite-analysis.md
```
...
```powershell
bun run cli analyze-binary --dir range-db/binary --sqlite-report reports/sqlite-analysis.json --out reports/binary-analysis.json --md reports/storage-analysis.md
```
...
```powershell
bun run cli verify --scheme scheme1
```
...
```powershell
bun run cli benchmark --scheme scheme1
```
